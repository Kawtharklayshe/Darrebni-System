...
// 👉 Store
const blogstore = useRoadmapstoreModule()
...
const deleteLang = (id: number) => {
  swal({
    ...
    if (result.value) {
      blogstore.Deleteroadmap(id).then(response => {
        swal({
          title: ' Deleted ',
          icon: 'success',
          customClass: {
        ...
        >
          ...
          <td style="width: 8rem;">
            <VBtn
              ...
              color="info"
              variant="text"
              :to="{ name: 'apps-roadmapmodule-edit-id', params: { id: item.id } }"
            >
              <VIcon
                size="22"
                icon="tabler-edit"
