...
roadmapStore.fetchroadmapById(Number(route.params.id)).then(response => {
  roadmap.value = response.data.data
})
...
const onSubmit = () => {
  refForm.value?.validate().then(({ valid }) => {
    if (valid) {
      loading.value = true
      roadmapStore.updateroadmap(roadmap.value).then(res => {
        swal({
          title: ' Updated ',
          icon: 'success',
          customClass: {
