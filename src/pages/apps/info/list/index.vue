...
const deleteLang = (id: number) => {
  swal({
    ...
    if (result.value) {
      infostore.Deleteinfo(id).then(response => {
        swal({
          title: ' Deleted ',
          icon: 'success',
          customClass: {
...
const paginationData = computed(() => {
  const firstIndex = infos.value.length ? ((currentPage.value - 1) * rowPerPage.value) + 1 : 0
  const lastIndex = infos.value.length + ((currentPage.value - 1) * rowPerPage.value)

  return `Showing ${firstIndex} to ${lastIndex} of ${totalinfos.value} entries`
})
</script>
